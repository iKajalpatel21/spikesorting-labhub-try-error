...
def process_job(job_data):
    ...
    try:
        num_completed_steps = 0
        # Corrected: Iterate through 'job_steps' which is the correct key
        for step in job_data["job_steps"]:
            # In the new qmodel.json format, we use identifier, not step_id
            identifier = step["identifier"]
            function_name = step["function"]
            
            # The step configuration is available as a top-level key
            step_config = job_data.get(identifier, {})

            logging.info(f"Processing step '{identifier}' (function: {function_name}) for Job {job_id}")
            logging.info(f"Step config: {step_config}")
            
            # Note: We can't update individual step status since we don't have step_id
            # We'll just update the main job status
            ...
            logging.info(f"Finished step '{identifier}'.")
            num_completed_steps += 1

        if num_completed_steps == len(job_data["job_steps"]):
            update_the_status(job_id, "finished")
            logging.info(f"Job {job_id} finished successfully.")
