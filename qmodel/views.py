...
def submit_nested_json_job(request):
    ...
    """
    print(f"DEBUG: FUNCTION CALLED - Method={request.method}")
    print(
        f"DEBUG: Method={request.method}, FILES={list(request.FILES.keys())}, POST={list(request.POST.keys())}"
    )

    if request.method == "POST":
        # Check if a file was uploaded. If not, set an error message.
        if not request.FILES.get("json_file"):
            print("DEBUG: No json_file found in request.FILES")
            messages.error(request, "❌ Error: Please select a JSON file to upload.")
            return redirect("qmodel:submit_json")

        json_file = request.FILES["json_file"]
        print(f"DEBUG: JSON file found: {json_file.name}")
        ...
        try:
            ...
            job_env_config = data.get("job_env", {})
            ...
            with transaction.atomic():
                ...
                print(f"DEBUG: Job created successfully with ID: {job.job_id}")
    ...
    # This part handles the initial GET request to display the form.
