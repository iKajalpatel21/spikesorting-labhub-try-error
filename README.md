...
2. Frontend/light-backend accepts job orders from the users. On the submit button, they create a new record in the database with the status `pending`. When a worker requests a job from `https://127.0.0.1/jobs`, they check if there is any recording with the `pending` flag, and if there is, they form JSON and send it to the worker. The state of the record should be changed to `fetched`. When a worker reports a `running`,`finished`, or `field` state, the record should be updated to this state. -- Toy example
