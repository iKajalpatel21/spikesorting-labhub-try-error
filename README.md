...
1. Worker fetches jobs from the backend, requesting the `https://127.0.0.1/jobs` URL every 10 seconds. If no new jobs are in the queue, it gets json with empty dictionary `{}`. If there is a job, it gets JSON with job parameters `{"jobid": "b5b126669b43", "a":12, "b":27}`, where `a' and `b` just parameters provided by user ar the job submission. As the job is received, the worker sends the job state to the backend by sending `{"jobid": "b5b126669b43", "state": "running"}` to the URL `https://127.0.0.1/jobs/b5b126669b43` and sleeps `a' seconds. After that, it sends job state `{"jobid": "b5b126669b43", "state": "finished"}` to the same URL. -- Toy example
